...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 22 00:23:47 2021
...
class MixQuantActiv(nn.Module):
    ...
    def __init__(self, bits):
        ...
        self.bits = bits
        self.gamma_activ = Parameter(torch.Tensor(len(self.bits)))
        self.gamma_activ.data.fill_(0.01)
    ...
    def forward(self, input):
        outs = []
        sw = F.softmax(self.gamma_activ, dim=0)
...
class MixQuantConv2d(nn.Module):
    ...
    def __init__(self, inplane, outplane, bits, **kwargs):
        ...
        self.bits = bits
        self.gamma_weight = Parameter(torch.Tensor(len(self.bits)))
        self.gamma_weight.data.fill_(0.01)
    ...
    def forward(self, input):
        mix_quant_weight = []
        sw = F.softmax(self.gamma_weight, dim=0)
...
class MixActivConv2d(nn.Module):
    ...
    def complexity_loss(self):
        sw = F.softmax(self.mix_activ.gamma_activ, dim=0)
        mix_abit = 0
        abits = self.mix_activ.bits
        for i in range(len(abits)):
            mix_abit += sw[i] * abits[i]
        sw = F.softmax(self.mix_weight.gamma_weight, dim=0)
        mix_wbit = 0
